...
public class GetDeviceId {
    ...
    public static String getAndroidId(final Context context) {
        try {
            SharedPreferencesHelper sharedPreferencesHelper = new SharedPreferencesHelper(context, "ANDROID_ID");
            //获取保存在sd中的 设备唯一标识符
            String readDeviceID = GetDeviceId.readDeviceID(context);
            //获取缓存在  sharepreference 里面的 设备唯一标识
            String string = (String) sharedPreferencesHelper.getSharedPreference("android_id", readDeviceID);
            ...
            if (!TextUtils.isEmpty(string)) {
                ...
                if (!TextUtils.isEmpty(readDeviceID)) {
                    ...
                    //左后再次更新app 的缓存
                    sharedPreferencesHelper.put("android_id", readDeviceID);
                    GetDeviceId.saveDeviceID(readDeviceID, context);
                }
                return readDeviceID;
            }
